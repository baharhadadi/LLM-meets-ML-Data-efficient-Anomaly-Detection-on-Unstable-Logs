...
def prepare_data(logID2Temp, templateVocab, dataset, fixLength, logger, ratio=[6, 1, 3], shuffle=True, output_dir=""):
    start = time.time()
    if dataset == 'bgl' or dataset == "spirit" or dataset == "tdb" or dataset == "hdfs" or dataset =="adfa" or dataset=="hadoop" or dataset=="spark":
...
def PULearn1(pre_train, pre_dev, pre_test, save_path, min_cluster_size=None, min_samples=None,
    ...
    if pre_train is not None and (not os.path.exists(os.path.join(save_path, 'HDBscan_result.txt')) or refresh):
        ...
        logger.info('Fit predict of dbscan Finished')
...
def main_process(save_path, pre_train, pre_dev, pre_test, ratios, hdbscan_option,
                 ...
                 threshold=0.5, dataset="BGL", logID2Temp={}, trained = False):
    mpl.use('Agg')
    ...
    if not trained :
        ...
        dev_p, dev_r, dev_sp, dev_f, final_p, final_r, final_sp, final_f = train_model(train, dev, test, classifier, vocab, config,
    ...
    #evaluate(test_data, bestClassifier, config, vocab, logger, outputFile=None,threshold=threshold)

    print("online evaluation:", res)
    return [dev_p, dev_r, dev_sp, dev_f]
...
def run_PLELog(options):
    ...
    if dataset == "hdfs" or dataset == "bgl" or dataset == "spirit" or dataset == "tdb" or dataset=="adfa" or dataset=="hadoop" or dataset=="spark":
        ...
        logger.info("Unknown dataset123")
        exit(-2)
    ...
    result = main_process(save_path=train_save_path, pre_train=pre_train, pre_dev=pre_dev, pre_test=pre_test,
                          ...
                          logger=logger,
                          threshold=threshold, dataset=dataset, logID2Temp=logID2Temp, trained=options["trained"])
