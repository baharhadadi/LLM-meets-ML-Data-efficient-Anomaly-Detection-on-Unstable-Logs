...
def train_model(data, dev, test_data, anomaly_detection, vocab, config, vec=None, logger=None,
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            ...
            if dev:
                if batch_iter % config.validate_every == 0 or batch_iter == batch_num:
                    ...
                    _, _, _, f = evaluate(dev, anomaly_detection, config, vocab, logger)
                    logger.info('\n')
                    if f > bestF:
    ...
    final_p, final_r, final_sp, final_f = evaluate(test_data, anomaly_detection, config, vocab, logger, threshold=threshold)
    if bestClassifier:
        ...
        dev_p, dev_r, dev_sp, dev_f = evaluate(test_data, bestClassifier, config, vocab, logger, outputFile=None,
                                       threshold=threshold)
    else:
        logger.info(
        ...
        dev_p, dev_r, dev_sp, dev_f = 0, 0, 0, 0
    return dev_p, dev_r, dev_sp, dev_f, final_p, final_r, final_sp, final_f
...
def evaluate_online(data, config, vocab, logger, vec={}, outputFile=None, threshold=0.5, id2tem={}, dat="BGL"):
    ...
    print("size of abnormal sequences in online evaluation:", len(abnormal_insts))
    if TP + FP != 0:
        precision = 100 * TP / (TP + FP)
        recall = 100 * TP / (TP + FN)
        f = 2 * precision * recall / (precision + recall)
        ...
        print('Precision = %d / %d = %.4f, Recall = %d / %d = %.4f Specificity = %d / %d = %.4f F1 score = %.4f, time = %.2f'
