...
def process_dataset(data_dir, output_dir, log_file, dataset_name, window_type, window_size, step_size, train_size,
        ...
        if random_sample:
            ...
            window_df = sliding(df[["timestamp", "Label", "EventId", "deltaT", "EventTemplate", "Content"]],
                                       para={"window_size": window_size,
                                             "step_size": step_size})
            window_df = shuffle(window_df, random_state=0).reset_index(drop=True)
    ...
    test_window = shuffle(test_window, random_state=0) #.reset_index(drop=True)
    #if len(test_window) > 2000:
    #  test_window = test_window[:2000]
    # save pickle file
