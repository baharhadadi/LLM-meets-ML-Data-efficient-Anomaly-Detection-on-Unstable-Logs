...
def sliding_window(data_iter, vocab, window_size, is_train=True, data_dir="dataset/", is_predict_logkey=True,
                   e_name="embeddings.json", semantics=True, sample_ratio=1, in_size=300):
    if e_name == "neural":
        event2semantic_vec = {}
        is_bert = True
        #data_iter = data_iter[:10000]
    else:
        if semantics:
          event2semantic_vec = read_json(os.path.join(data_dir, e_name))
          #vocab.semantic_vectors = read_json(os.path.join(data_dir, e_name))
          #vocab.semantic_vectors["padding"] = [-1] * in_size
        is_bert = False
    result_logs = []
    labels = []
    ...
    if len(data_iter[0]) < 3:
        data_iter = [(x, y , []) for (x, y) in  data_iter ]
        ...
        for i in range(len(line) - window_size if is_predict_logkey else len(line) - window_size + 1):
            ...
            if semantics:
                ...
                for event in seq_logs:
                    if event == "padding":
                        if not is_bert: in_size = 300
                        if is_bert: in_size = 768
