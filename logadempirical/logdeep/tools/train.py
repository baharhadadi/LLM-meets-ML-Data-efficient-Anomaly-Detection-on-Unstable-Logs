...
class Trainer():
    def __init__(self, options):
        ...
        if self.sample == 'sliding_window':
            ...
            #n_train = int(len(data) * self.train_size*0.001)
            
            if self.model_name in ["loganomaly", "deeplog"]:
            ...
            if len(train_logs) > 200000 and self.model_name in ["logrobust", "plelog", "neurallog"]:
                    train_logs = train_logs[:200000]
                    train_labels = train_labels[:200000]
            else:
                if self.model_name in ["neurallog"] and self.dataset_name in ["spark"]:
                    train_labels = train_labels[:int(len(train_labels))]
                    train_logs = train_logs[:int(len(train_logs))]
                    
            print(len(train_logs), len(train_labels) )
        ...
        #  if torch.isnan(t["features"][2]).any():
        #    print("Nan detected")
        #    break
