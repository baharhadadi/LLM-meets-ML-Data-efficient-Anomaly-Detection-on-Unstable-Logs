...
def generate(output_dir, name, name_train="train.pkl", is_neural=False, is_dedup = True):
    print("Loading", output_dir + name)
    with open(output_dir + name, 'rb') as f:
        data_iter = pickle.load(f)

    print("Loading", output_dir + name_train)
    with open(output_dir + name_train, 'rb') as f:
        data_train_iter = pickle.load(f)

    unique_seqs = set()
    for seq in data_train_iter:
      unique_seqs.add(tuple(seq["EventId"]))

    normal_iter = {}
    abnormal_iter = {}
    for seq in data_iter:
      if tuple(seq["EventId"]) in unique_seqs and is_dedup==True:
        continue
      else:
...
class Predicter():
    ...
    def semi_supervised_helper(self, model, logs, vocab, data_type, scale=None, min_len=0):
        ...
        #batch_size_calculated = min(len(dataset), 512)
        batch_size_calculated = 1
    ...
    def predict_supervised(self):
        ...
        if self.model_name == "cnn":
            ...
            model_init = lstm_model(sequence_length = self.input_size,
        ...
        with open(self.output_dir + "predict_supervised_results.pkl", 'wb') as f:
          pickle.dump(result_dic, f)

    def predict_supervised_batch(self):
        with open(self.vocab_path, 'rb') as f:
            vocab = pickle.load(f)
        ...
        seqs, labels = sliding_window(sess_normal_events, vocab, window_size=self.history_size, is_train=False,
                                      is_predict_logkey=False, e_name=self.embeddings,
                                      data_dir=self.data_dir, semantics=self.semantics)
        ...
        total_normal += len(sess_normal_events)
        for i, (log, label) in enumerate(tbar):
            with torch.no_grad():
                ...
                if isinstance(output, dict):
                    ...
                    label = np.array([y.cpu().numpy() for y in label])
                    FP += (predicted != label).sum()
        ...
        data_loader = DataLoader(dataset,
                                 ...
                                 pin_memory=True)
        tbar = tqdm(data_loader)
        total_abnormal += len(sess_abnormal_events)
        for i, (log, label) in enumerate(tbar):
            with torch.no_grad():
                del log['idx']
                features = [x.to(self.device) for x in log['features']]
        ...
        with open(self.output_dir + self.model_name + "-leadtime.txt", mode="w") as f:
            [f.write(str(i) + "\n") for i in lead_time]
        print('Precision: {:.3f}%, Recall: {:.3f}%, F1-measure: {:.3f}%, '
              'Specificity: {:.3f}, Lead time: {}'.format(P, R, F1, SP, lead_time))
