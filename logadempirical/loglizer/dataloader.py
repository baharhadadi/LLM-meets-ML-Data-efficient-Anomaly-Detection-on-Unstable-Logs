...
def load_preprocessed(data_dir, template_dir):

    logtemplate_file = pd.read_csv(template_dir)
    # Convert EventId to string if it's an integer type
    if logtemplate_file["EventId"].dtype == 'int64':
        logtemplate_file["EventId"] = logtemplate_file["EventId"].astype(str)
    logtemplate = dict(zip(logtemplate_file["EventId"], logtemplate_file["EventTemplate"]))
    ...
    train_y = []
    uniques = set()
    for element in train_window:
      logsequence = []
      for eventid in element["EventId"]:
        logsequence.append(logtemplate[eventid])
      train_x.append(logsequence)
      train_y.append(element["Label"])
      uniques.add(tuple(element["EventId"]))
    
    with open(data_dir+"/test.pkl", 'rb') as f:
        test_window = pickle.load(f)

    test_x = []
    test_y = []
    for element in test_window:
      logsequence = []
      for eventid in element["EventId"]:
        logsequence.append(logtemplate[eventid])
      if tuple(element["EventId"]) in uniques:
          continue
      test_x.append(logsequence)
      test_y.append(element["Label"])
